    ...
    <div class="pageback">
      ...
      <div class="pagepadding">
        ...
        <el-button
          ...
          type="warning"
          style="margin-left: 16px;margin-bottom: 16px;"
          @click="newDialog = true">新增/修改全局变量</el-button>
...
onMounted(async () => {
  ...
  searchNameList.value.push({name: '名称',label: 'key'});
  searchNameList.value.push({name: '内容',label: 'value'});
  userNameList.value = [];
...
const getList = async () => {
  try{
    ...
    const { result } = await api.post('/searchglobal', postbody);
    console.log(result);
...
const newUser = async () => {
  try{
    ...
    const result = await api.post('/addglobal', {
